    ...
    public partial class AuthForm : Form
    {
        ...
        private async void ResendButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    // send verification code to email
                    var email = await InstaApi.RequestVerifyCodeToEmailForChallengeRequireAsync(true);
                    if (email.Succeeded)
                    {
                        LblForSmsEmail.Text =
                ...
                {
                    // send verification code to phone number
                    var phoneNumber = await InstaApi.RequestVerifyCodeToSMSForChallengeRequireAsync(true);
                    if (phoneNumber.Succeeded)
                    {
                        LblForSmsEmail.Text =
        ...
        private async void VerifyButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Verification code must be 6 digits!!!", "ERR", MessageBoxButtons.OK,
